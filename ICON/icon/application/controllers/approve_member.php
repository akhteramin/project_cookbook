...
class Approve_member extends CI_Controller {
	...
	try {
            ...
            $crud->set_subject('User');
			  $crud->columns('student_name','username','email','area');
			   $crud->add_action('Approve', '', '','ui-icon-image',array($this,'approve_request'));
			 //$crud->callback_column('username',array($this,'_callback_webpage_url'));
            $crud->unset_add();
			$crud->unset_edit();
			   $crud->where('active',0);
            $output = $crud->render();
	...
	$slash="approve_member/show_membership/";
	     return site_url($slash.$row->username);
	}
	
	public function show_membership($username)
	{
	...
	$this->approve_member_model->activate_member($username);
